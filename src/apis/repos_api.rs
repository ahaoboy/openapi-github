...
/// struct for typed errors of method [`repos_slash_compare_commits_with_basehead`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ReposSlashCompareCommitsWithBaseheadError {
    Status404(models::BasicError),
    Status500(models::BasicError),
...
pub async fn repos_slash_compare_commits(configuration: &configuration::Configuration, owner: &str, repo: &str, base: &str, head: &str, per_page: Option<i32>, page: Option<i32>) -> Result<models::CommitComparison, Error<ReposSlashCompareCommitsError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/repos/{owner}/{repo}/compare/{base}...{head}", local_var_configuration.base_path, owner=crate::apis::urlencode(owner), repo=crate::apis::urlencode(repo), base=crate::apis::urlencode(base), head=crate::apis::urlencode(head));
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
...
pub async fn repos_slash_compare_commits_with_basehead(configuration: &configuration::Configuration, owner: &str, repo: &str, basehead: &str, page: Option<i32>, per_page: Option<i32>) -> Result<models::CommitComparison, Error<ReposSlashCompareCommitsWithBaseheadError>> {
    let local_var_configuration = configuration;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<ReposSlashCompareCommitsWithBaseheadError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
...
pub async fn repos_slash_get_commit_comment(configuration: &configuration::Configuration, owner: &str, repo: &str, comment_id: i32) -> Result<models::CommitComment, Error<ReposSlashGetCommitCommentError>> {
    let local_var_configuration = configuration;
...
pub async fn repos_slash_update_commit_comment(configuration: &configuration::Configuration, owner: &str, repo: &str, comment_id: i32, repos_update_commit_comment_request: models::ReposUpdateCommitCommentRequest) -> Result<models::CommitComment, Error<ReposSlashUpdateCommitCommentError>> {
    let local_var_configuration = configuration;
